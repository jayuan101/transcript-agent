...
9 STT engines · 8 AI providers · real-time video analysis · local-first.
...
| 🤖 **AI analysis** | Claude, OpenAI, Gemini, Groq, Mistral, Together AI, Perplexity, **Ollama (local — default: gemma3:27b)** |
| 🗣️ **37+ languages** | Auto-detect or choose, with regional dialect variants |
| 🎯 **Interview coaching** | Per-question scoring (Great / Good / Needs Improvement / Missed), 10-point score, coaching tips, advancement likelihood |
| 🎥 **Video Analysis** | Upload a recorded interview — detects emotion, eye contact, posture, body language per person. Score cards, emotion timeline, annotated video download |
| 🔴 **Live Interview** | Real-time webcam analysis — live emotion, eye contact, body language badge, scores update every 5 seconds |
| 💪 **Body language** | Arm crossing, forward lean, shoulder tension, head nod — OPEN / ENGAGED / TENSE / CLOSED badge |
| 🌎 **Cultural analysis** | American Interview Standard score + Indian → American adaptation score with coaching tips |
| 📊 **Reports** | Summary, key points, action items, speaker profiles, speech analytics |
| 📁 **History** | Every session saved locally — tokens, cost, score, full Q&A replay |
...
**Requirements:** Python 3.10+ from [python.org](https://www.python.org/downloads/) — check **"Add Python to PATH"** during install.
...
To launch again later — just double-click `setup_windows.bat` and choose **[1] Launch app**.
...
3. Click **Scan Faces** — the app detects how many people are in the video
...
1. Click the **🔴 Live Interview** tab
2. Set Person 1 / Person 2 roles
3. Choose Cultural Context (American / Indian → American / Both)
...
2. Pull the recommended model:
   ...
   ```
3. In the app: select **Ollama (Local)** as provider → model defaults to `gemma3:27b`
4. No API key needed
...
video_analyzer.py    — Video/webcam analysis: emotion, body language, cultural scoring
api.py               — FastAPI REST API (port 8000)
launcher.py          — Desktop app entry point (auto-opens browser)
