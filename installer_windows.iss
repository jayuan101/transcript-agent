...
[Icons]
Name: "{autodesktop}\{#AppName}"; Filename: "{app}\Launch-TranscriptAgent.bat"; WorkingDir: "{app}"; IconFilename: "{app}\TranscriptAgent\TranscriptAgent.exe"; Comment: "Transcript Agent - AI transcription and interview analysis"; Tasks: desktopicon
Name: "{group}\{#AppName}"; Filename: "{app}\Launch-TranscriptAgent.bat"; WorkingDir: "{app}"; IconFilename: "{app}\TranscriptAgent\TranscriptAgent.exe"

[UninstallDelete]
Type: files; Name: "{app}\version.txt"
Type: files; Name: "{app}\gpu_hint.txt"

[Run]
Filename: "{app}\Launch-TranscriptAgent.bat"; Description: "Launch {#AppName} now"; Flags: postinstall shellexec skipifsilent nowait

[Messages]
WelcomeLabel1=Welcome to {#AppName} Setup
WelcomeLabel2=This will install Transcript Agent {#AppVersion} on your computer.%n%nThe app opens in your browser at http://localhost:7860.%n%nUpdates are automatic on every launch.%n%nClick Next to continue.
FinishedLabel=Transcript Agent has been installed.%n%nClick Finish to launch it now.

[Code]
procedure CurStepChanged(CurStep: TSetupStep);
var
  VersionFile, GpuFile: string;
...
begin
  ...
  begin
    ...
    GpuFile := ExpandConstant('{app}\gpu_hint.txt');
    SaveStringToFile(GpuFile, GpuHint, False);
  end;
end;
